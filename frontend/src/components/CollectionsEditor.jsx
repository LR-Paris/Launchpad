...
export default function CollectionsEditor({ slug, locked = false }) {
  ...
  // ── Items: load when collection selected ────────────────────────────────
  const loadItems = useCallback(async (colName) => {
    ...
    try {
      const data = await listShopFiles(slug, colPath);
      ...
      const itemsData = await Promise.all(itemDirs.map(async (dir) => {
        ...
        try {
          ...
          const sorted = sortPhotos((photos.entries || []).filter(e => e.isImage));
  ...
  const handleRenameItem = async (oldDir, newName) => {
    ...
    try {
      ...
      if (editingItem === oldDir) setEditingItem(trimmed);
      setRenaming(null);
      flashSuccess(`Renamed to "${trimmed}". Inventory updated.`);
    } catch (err) {
      setError(err.response?.data?.error || 'Rename failed');
  ...
  const handleSetMainPhoto = async (photo) => {
    ...
    try {
      ...
      const affected = items.find(i => editingItem === i.dirName);
      if (affected) {
  ...
  const handleMoveItems = async (targetCollection, ids) => {
    ...
    if (movedCount > 0) {
      ...
      if (editingItem && ids.includes(editingItem)) setEditingItem(null);
      setSelectedIds(new Set());
      setBulkMode(false);
      setMoveTarget(null);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      const idx = visibleItems.findIndex(i => i.dirName === editingItem);
      if (e.key === 'j') {
        ...
        const target = items.find(i => i.dirName === editingItem);
        if (target && !disabled) handleDuplicateItem(target);
      } else if (e.key === 'm' && editingItem) {
        if (!disabled) setMoveTarget({ ids: [editingItem] });
              ...
              {!selectedCollection ? (
                ...
                <div className="p-5">
                  ...
                  {visibleItems.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4 group">
                      {visibleItems.map(item => {
                        const issues = validateItem({
                          details: { 'Name.txt': item.name, 'ItemCost.txt': item.price },
                        ...
                        return (
                          <DraggableItemCard
                            ...
                            isExpanded={editingItem === item.dirName}
                            isSelected={selectedIds.has(item.dirName)}
                            bulkMode={bulkMode}
                            disabled={disabled}
                            validationIssues={issues}
                            onClick={() => editingItem === item.dirName ? setEditingItem(null) : loadItemDetails(item)}
                            onToggleSelect={() => {
                              setSelectedIds(prev => {
                  ...
                  {editingItem && (
                    <div className="border-t border-border/40 pt-5 mt-2">
                      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                        <div className="flex items-center gap-2">
                          {renaming?.kind === 'item' && renaming.original === editingItem ? (
                            <input
                              autoFocus
                              value={renaming.value}
                              onChange={(e) => setRenaming({ ...renaming, value: e.target.value })}
                          ...
                          {editingItemValidation.length > 0 && (
