 ...
 */
/**
 * Look up a checkout field's own label from the shop's Checkout/schema.json.
 *
 * A field id is stable but its wording is the client's. ELC relabelled the
 * in-hands date to "Ex-Works Date Requested" because for them it means
 * ready-for-pick-up at the factory, and the e-mail has to say the same thing
 * the form said. Any shop without a schema, or without that field, keeps the
 * fallback, so this is inert everywhere else.
 */
function schemaFieldLabel(slug, fieldId, fallback) {
  if (!slug) return fallback;
  try {
    const p = path.join(SHOPS_DIR, slug, 'DATABASE', 'Checkout', 'schema.json');
    const schema = JSON.parse(fs.readFileSync(p, 'utf8'));
    for (const section of schema.sections || []) {
      for (const f of section.fields || []) {
        if (f.id === fieldId && f.label) {
          // "Ex-Works Date Requested" is a form prompt; an e-mail row wants the noun.
          return String(f.label).replace(/\s+Requested$/i, '').trim() || fallback;
        }
      }
    }
  } catch { /* no schema, unreadable, or field absent - use the fallback */ }
  return fallback;
}
...
function buildInfoBlock(row, slug) {
  const fields = [];
  ...
  const jobTitle = customField(row, 'jobTitle', 'Title', 'job_title');
  const department = customField(row, 'department', 'Department');
  const shipTerms = customField(row, 'shippingTerms', 'Shipping Terms', 'shipping_terms');
  const billContact = customField(row, 'billingContactName', 'Billing Contact Name');
  const billEmail = customField(row, 'billingEmail', 'Billing Contact Email');
  const billPhone = customField(row, 'billingPhone', 'Billing Contact Phone');
  const inHandLabel = schemaFieldLabel(slug, 'inHandDate', 'In Hand Date');
  ...
  if (jobTitle) fields.push(['Title', jobTitle]);
  ...
  if (shipTerms) fields.push(['Shipping Terms', shipTerms]);
  if (billing && billing !== address) fields.push(['Bill To', billing]);
  if (billContact) fields.push(['Billing Contact', billContact]);
  if (billEmail) fields.push(['Billing Email', billEmail]);
  if (billPhone) fields.push(['Billing Phone', billPhone]);
  if (inHand) fields.push([inHandLabel, formatDate(inHand) || inHand]);
...
async function sendOrderConfirmation(orderData, shopSlug) {
  ...
  const body = `
    ...
    ${buildInfoBlock(orderData, shopSlug)}
    ${cancelUrl ? `
...
function sendAdminOrderEmail(orderData, shopSlug, companyName, primaryColor, fromAddress, adminEmail) {
  ...
  const adminTitle = customField(orderData, 'jobTitle', 'Title', 'job_title');
  const adminDept = customField(orderData, 'department', 'Department');
  const adminShipTerms = customField(orderData, 'shippingTerms', 'Shipping Terms', 'shipping_terms');
  const adminBillContact = customField(orderData, 'billingContactName', 'Billing Contact Name');
  const adminBillEmail = customField(orderData, 'billingEmail', 'Billing Contact Email');
  const adminBillPhone = customField(orderData, 'billingPhone', 'Billing Contact Phone');
  const adminInHandLabel = schemaFieldLabel(shopSlug, 'inHandDate', 'In Hand');
  ...
  if (adminTitle) contactRows.push(['Title', adminTitle]);
  if (adminDept) contactRows.push(['Department', adminDept]);
  ...
  if (adminInHand) contactRows.push([adminInHandLabel, formatDate(adminInHand) || adminInHand]);
  ...
  if (adminBillContact) contactRows.push(['Billing Contact', adminBillContact]);
  if (adminBillEmail) contactRows.push(['Billing Email', `<a href="mailto:${esc(adminBillEmail)}" style="color:${primaryColor};text-decoration:none;">${esc(adminBillEmail)}</a>`]);
  if (adminBillPhone) contactRows.push(['Billing Phone', `<a href="tel:${esc(adminBillPhone)}" style="color:${primaryColor};text-decoration:none;">${esc(adminBillPhone)}</a>`]);
  ...
  if (adminShipTerms) shipFields.push(['Terms', adminShipTerms]);
