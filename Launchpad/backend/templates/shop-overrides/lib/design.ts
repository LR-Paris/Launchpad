  ...
  try {
    ...
    for (const ext of formats) {
      ...
      if (fs.existsSync(logoPath)) {
        return `${BASE_PATH}/api/images/logos/${filename}`;
      }
    }
    return null;
  } catch (error) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stats.isFile() && formats.test(file)) {
        heroImages.push(`${BASE_PATH}/api/images/showcase/${file}`);
      }
    }
  ...
  try {
    ...
    for (const file of files) {
      if (/\.(png|jpg|jpeg|webp|gif)$/i.test(file)) {
        ...
        showcaseImages[collectionId] = `${BASE_PATH}/api/images/showcase/Collections/${file}`;
      }
    }
